...
router.post("/", authCheck, jwtAuthz(['read:list-tables','write:list-tables']), (req, res, next) => {
  AlertLanding.destroy({
    truncate: true
  }).then(() => {
    AlertLanding.create(req.body)
      .then(data => {
        res.sendStatus(200);
      })
      .catch((err) => {
