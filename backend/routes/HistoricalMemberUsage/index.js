...
router.get('/well-depletions/:wells/:depletion_start_year', (req, res, next) => {
...
router.post('/views', async (req, res, next) => {
  ...
  try {
    await Historical_Member_Usage_Views.update(
      { view_default: false },
      {
        where: {
    ...
    const response = await Historical_Member_Usage_Views.upsert(data, { returning: true });
