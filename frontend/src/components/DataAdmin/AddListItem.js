...
const AddListItem = (props) => {
  ...
  useEffect(() => {
    ...
    formConfig.forEach((d) => {
      if (d.component === 'checkbox') {
        formValues[d.name] = false;
      } else {
        formValues[d.name] = '';
      }
    });
  ...
  /**
   * Utility function used to reset the form
   */
  const resetForm = () => {
    const formValues = {};
    formConfig.forEach((d) => {
      ...
      <div className={classes.content}>
        <section className={classes.row}>
          <div className={classes.pageTitleBar}>
            <Typography variant="h5" color="secondary" className={classes.title}>Add New Item</Typography>
