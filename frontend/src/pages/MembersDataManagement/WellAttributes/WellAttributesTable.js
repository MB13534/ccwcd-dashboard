...
const WellAttributesTable = ({ refreshSwitch, meters, showInactive }) => {
  const classes = useStyles();
  const { setWaitingState, snackbarOpen, snackbarError, handleSnackbarClose } = useFormSubmitStatus();
  const { getTokenSilently } = useAuth0();

  const [tableData, isLoading, setTableData] = useFetchData('members-management/well-attributes', [refreshSwitch]);
  ...
  const submitUpdate = async record => {
    ...
    try {
      ...
      await axios.put(`${process.env.REACT_APP_ENDPOINT}/api/members-management/well-attributes`, record, { headers });
  ...
  const submitDelete = async record => {
    ...
    try {
      const token = await getTokenSilently();
      const headers = { Authorization: `Bearer ${token}` };
      await axios.put(`${process.env.REACT_APP_ENDPOINT}/api/members-management/well-attributes`, rec, { headers });
      setWaitingState('complete', 'no error');
    } catch (err) {
      console.error(err);
      setWaitingState('complete', 'error');
  ...
  const filterData = data => {
    if (showInactive === false) {
      return data.filter(item => !item.inactive);
    }
    return data;
  };
