...
const EditStructureGroups = (props) => {
  ...
  const [StructureGroupData] = useFetchData(`structure-groups/${match.params.id}`, []);
  ...
  useEffect(() => {
    if (StructureGroupData.length !== 0) {
      const {
        group_ndx,
        group_name,
        centroid_lon_dd,
      ...
      setMarker((marker) => {
        ...
        const newMarker = new mapboxgl.Marker()
          .setLngLat([StructureGroupData.centroid_lon_dd, StructureGroupData.centroid_lat_dd])
          .addTo(map);
        map.setCenter([StructureGroupData.centroid_lon_dd, StructureGroupData.centroid_lat_dd])
        return newMarker;
      });
  ...
  }, [StructureGroupData]); //eslint-disable-line
      ...
      <div className={classes.content}>
        <section className={classes.row}>
          <div className={classes.pageTitleBar}>
            <Typography variant="h5" color="secondary" className={classes.title}>Edit Structure Group</Typography>
            ...
            <Button variant="contained" size="small" color="secondary" style={{marginLeft: 15}} component={AdapterLink} to="/admin/data/structure-groups/new">+ Add New</Button>
          </div>
...
EditStructureGroups.propTypes = {
  auth: PropTypes.object.isRequired,
  history: PropTypes.object.isRequired,
};

export default withStyles(styles, { withTheme: true })(EditStructureGroups);
