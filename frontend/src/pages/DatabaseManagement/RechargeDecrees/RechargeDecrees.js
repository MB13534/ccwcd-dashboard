...
const RechargeDecrees = (props) => {
  const classes = useStyles();
  const [Data, isLoading, setData] = useFetchData("recharge-decrees", []);

  const Columns = [{ title: "Description", field: "recharge_decree_desc" }];
      ...
      <section className={classes.root}>
        <div className={classes.content}>
          <Container maxWidth="lg" className={classes.container}>
            ...
            <DataAdminTable
              title="Recharge Decrees Management"
              data={Data}
              columns={Columns}
              loading={isLoading}
              updateHandler={setData}
              endpoint="recharge-decrees"
              ndxField="recharge_decree_ndx"
            />
          </Container>
        </div>
      </section>
...
export default RechargeDecrees;
