...
const RechargeProjects = (props) => {
  const classes = useStyles();
  const [Data, isLoading, setData] = useFetchData("recharge-projects", []);

  const Columns = [{ title: "Description", field: "recharge_project_desc" }];
      ...
      <section className={classes.root}>
        <div className={classes.content}>
          <Container maxWidth="lg" className={classes.container}>
            ...
            <DataAdminTable
              title="Recharge Projects Management"
              data={Data}
              columns={Columns}
              loading={isLoading}
              updateHandler={setData}
              endpoint="recharge-projects"
              ndxField="recharge_project_ndx"
            />
          </Container>
        </div>
      </section>
...
export default RechargeProjects;
