...
const RechargeDataExport = props => {
  ...
  const [UnlaggedRechargeSlices, isLoading] = useFetchData('recharge-accounting/flags/unlagged', []);
  ...
  const handleExport = async event => {
    ...
    try {
      ...
      await axios.post(`${process.env.REACT_APP_ENDPOINT}/api/recharge-accounting/export`, {}, { headers });
      ...
      <Paper elevation={0} className={classes.paper}>
        ...
        <InfoCard mt={2}>
          <Typography variant="body1">
            The last part of the import process is to export the lagged data. Do so by clicking the "Export Data" button
            below.
          </Typography>
        </InfoCard>
        <Box mt={2} mb={2}>
          ...
          <Box mt={2} mb={2}>
            <Button variant="contained" component={Link} to="/recharge-accounting/data/process/lag">
              Back
            </Button>
            <Button
              size="large"
